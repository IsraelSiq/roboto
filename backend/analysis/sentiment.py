"""
Roboto — Sentiment Analyzer (FinBERT)
...
Arquitetura:
    ...
    - Fonte de notícias: CryptoPanic (público, sem API key) + RSS fallback
...
class SentimentAnalyzer:
    """
    Analisa sentiment de notícias usando FinBERT.

    Args:
        model_name:     Modelo HuggingFace a usar
        min_confidence: Score mínimo para aceitar sinal não-neutro
        max_headlines:  Máximo de manchetes a analisar por ciclo
        cache_ttl:      Tempo de vida do cache em segundos (0 = desabilita)
    """
    ...
    def _load_model(self):
        """Carrega o pipeline FinBERT (lazy loading)."""
    ...
    def analyze_news(self, news_list: list[dict]) -> SentimentResult:
        """
        Analisa uma lista de notícias e retorna o sentiment agregado.
    ...
    def _get_cache(self, key: str) -> Optional[SentimentResult]:
        ...
        if self.cache_ttl > 0 and (time() - ts) > self.cache_ttl:
            del self._cache[key]
            return None
        return result
